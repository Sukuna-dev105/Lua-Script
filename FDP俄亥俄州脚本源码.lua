...
makecharbutton("秒拿空投（开一次即可）").MouseButton1Click:Connect(function()
	while true do
		wait(5)
        ...
        if obj.ClassName == "Model" then
            for _, innerObj in pairs(obj:GetChildren()) do
                if innerObj.ClassName == "Model" then
                    ...
                    elseif innerObj.Name == "Amethyst" then
                        if innerObj:FindFirstChild("Handle") and innerObj.Handle:FindFirstChild("ProximityPrompt") then
                            innerObj.Handle.ProximityPrompt.HoldDuration = 0                                end
                        if innerObj:FindFirstChild("Handle") and innerObj.Handle:FindFirstChild("ProximityPrompt") then
                            innerObj.Handle.ProximityPrompt.HoldDuration = 0
                        end
                    elseif innerObj.Name == "Diamond" then
...
local Dealer1 = makecharbutton("远程黑市（关）")
...
makecharbutton("原皮枪连发无后座（开一次即可）").MouseButton1Click:Connect(function()
    while wait(60) do
...
local hitbox123 = makecharbutton("近战范围(关)")
...
local espitem = makecharbutton("透视物品(关)")
...
local noclipb = makecharbutton("穿墙(关)")
local noclip = false
...
local infjumpb = makecharbutton("连跳(关)")
local infjump = false
local con = nil
...
makecharbutton("隐身(X开关)").MouseButton1Click:Connect(function()
...
local automoneyprint = makeutilbutton("自动捡印钞机(关)")
...
local autoairdrop123 = makeutilbutton("自动捡空投、空袭标记(关)")
...
autoairdrop123.MouseButton1Click:Connect(function()
    if autoairdrop123t == false then
        ...
        autoairdrop123.Text = "自动捡空投、空袭标记(关)"
    end
